<?php 
return array (
  '_fromCache' => 'disk',
  '_initiator' => 
  array (
    'type' => 'script',
    'stack' => 
    array (
      'callFrames' => 
      array (
        0 => 
        array (
          'functionName' => 'a',
          'scriptId' => '582',
          'url' => 'https://static.cdninstagram.com/rsrc.php/v3/yL/r/gzOEkp8ap8N.js?_nc_x=Ij3Wp8lg5Kz',
          'lineNumber' => 59,
          'columnNumber' => 337,
        ),
        1 => 
        array (
          'functionName' => 'loadResources',
          'scriptId' => '582',
          'url' => 'https://static.cdninstagram.com/rsrc.php/v3/yL/r/gzOEkp8ap8N.js?_nc_x=Ij3Wp8lg5Kz',
          'lineNumber' => 170,
          'columnNumber' => 16915,
        ),
        2 => 
        array (
          'functionName' => 'handle',
          'scriptId' => '582',
          'url' => 'https://static.cdninstagram.com/rsrc.php/v3/yL/r/gzOEkp8ap8N.js?_nc_x=Ij3Wp8lg5Kz',
          'lineNumber' => 217,
          'columnNumber' => 969,
        ),
        3 => 
        array (
          'functionName' => 'x',
          'scriptId' => '583',
          'url' => 'https://static.cdninstagram.com/rsrc.php/v3ia5j4/yv/l/makehaste_jhash/RhNRE4yScUOVOjCpYeH3K9uqB-lvqIwSmBnOPSgWuQGVD1sdS7Mymz79l_WWzU_RmJBYBHirbomYfWbhZY3DGxUDcCT8qjcFOaQYb9xNH8B7YuZOJBbMsvnwWydCeTPmFLeaS3NUnsyKbue2m_T8waxN0Wi8b_8ZbsILoH6X7_0-thwdWgREOK9iy7Q9wVR-aBtoVB3cK-ZB0zezH.js?_nc_x=Ij3Wp8lg5Kz',
          'lineNumber' => 684,
          'columnNumber' => 4167,
        ),
        4 => 
        array (
          'functionName' => 's',
          'scriptId' => '583',
          'url' => 'https://static.cdninstagram.com/rsrc.php/v3ia5j4/yv/l/makehaste_jhash/RhNRE4yScUOVOjCpYeH3K9uqB-lvqIwSmBnOPSgWuQGVD1sdS7Mymz79l_WWzU_RmJBYBHirbomYfWbhZY3DGxUDcCT8qjcFOaQYb9xNH8B7YuZOJBbMsvnwWydCeTPmFLeaS3NUnsyKbue2m_T8waxN0Wi8b_8ZbsILoH6X7_0-thwdWgREOK9iy7Q9wVR-aBtoVB3cK-ZB0zezH.js?_nc_x=Ij3Wp8lg5Kz',
          'lineNumber' => 684,
          'columnNumber' => 843,
        ),
        5 => 
        array (
          'functionName' => '',
          'scriptId' => '583',
          'url' => 'https://static.cdninstagram.com/rsrc.php/v3ia5j4/yv/l/makehaste_jhash/RhNRE4yScUOVOjCpYeH3K9uqB-lvqIwSmBnOPSgWuQGVD1sdS7Mymz79l_WWzU_RmJBYBHirbomYfWbhZY3DGxUDcCT8qjcFOaQYb9xNH8B7YuZOJBbMsvnwWydCeTPmFLeaS3NUnsyKbue2m_T8waxN0Wi8b_8ZbsILoH6X7_0-thwdWgREOK9iy7Q9wVR-aBtoVB3cK-ZB0zezH.js?_nc_x=Ij3Wp8lg5Kz',
          'lineNumber' => 684,
          'columnNumber' => 1491,
        ),
      ),
      'parent' => 
      array (
        'description' => 'Promise.then',
        'callFrames' => 
        array (
          0 => 
          array (
            'functionName' => 'u',
            'scriptId' => '583',
            'url' => 'https://static.cdninstagram.com/rsrc.php/v3ia5j4/yv/l/makehaste_jhash/RhNRE4yScUOVOjCpYeH3K9uqB-lvqIwSmBnOPSgWuQGVD1sdS7Mymz79l_WWzU_RmJBYBHirbomYfWbhZY3DGxUDcCT8qjcFOaQYb9xNH8B7YuZOJBbMsvnwWydCeTPmFLeaS3NUnsyKbue2m_T8waxN0Wi8b_8ZbsILoH6X7_0-thwdWgREOK9iy7Q9wVR-aBtoVB3cK-ZB0zezH.js?_nc_x=Ij3Wp8lg5Kz',
            'lineNumber' => 684,
            'columnNumber' => 1460,
          ),
          1 => 
          array (
            'functionName' => 'a',
            'scriptId' => '583',
            'url' => 'https://static.cdninstagram.com/rsrc.php/v3ia5j4/yv/l/makehaste_jhash/RhNRE4yScUOVOjCpYeH3K9uqB-lvqIwSmBnOPSgWuQGVD1sdS7Mymz79l_WWzU_RmJBYBHirbomYfWbhZY3DGxUDcCT8qjcFOaQYb9xNH8B7YuZOJBbMsvnwWydCeTPmFLeaS3NUnsyKbue2m_T8waxN0Wi8b_8ZbsILoH6X7_0-thwdWgREOK9iy7Q9wVR-aBtoVB3cK-ZB0zezH.js?_nc_x=Ij3Wp8lg5Kz',
            'lineNumber' => 684,
            'columnNumber' => 2056,
          ),
          2 => 
          array (
            'functionName' => '',
            'scriptId' => '583',
            'url' => 'https://static.cdninstagram.com/rsrc.php/v3ia5j4/yv/l/makehaste_jhash/RhNRE4yScUOVOjCpYeH3K9uqB-lvqIwSmBnOPSgWuQGVD1sdS7Mymz79l_WWzU_RmJBYBHirbomYfWbhZY3DGxUDcCT8qjcFOaQYb9xNH8B7YuZOJBbMsvnwWydCeTPmFLeaS3NUnsyKbue2m_T8waxN0Wi8b_8ZbsILoH6X7_0-thwdWgREOK9iy7Q9wVR-aBtoVB3cK-ZB0zezH.js?_nc_x=Ij3Wp8lg5Kz',
            'lineNumber' => 684,
            'columnNumber' => 2452,
          ),
          3 => 
          array (
            'functionName' => 'applyWithGuard',
            'scriptId' => '582',
            'url' => 'https://static.cdninstagram.com/rsrc.php/v3/yL/r/gzOEkp8ap8N.js?_nc_x=Ij3Wp8lg5Kz',
            'lineNumber' => 69,
            'columnNumber' => 10431,
          ),
          4 => 
          array (
            'functionName' => 'c',
            'scriptId' => '582',
            'url' => 'https://static.cdninstagram.com/rsrc.php/v3/yL/r/gzOEkp8ap8N.js?_nc_x=Ij3Wp8lg5Kz',
            'lineNumber' => 69,
            'columnNumber' => 11165,
          ),
          5 => 
          array (
            'functionName' => 'O',
            'scriptId' => '582',
            'url' => 'https://static.cdninstagram.com/rsrc.php/v3/yL/r/gzOEkp8ap8N.js?_nc_x=Ij3Wp8lg5Kz',
            'lineNumber' => 228,
            'columnNumber' => 2291,
          ),
          6 => 
          array (
            'functionName' => 'U',
            'scriptId' => '582',
            'url' => 'https://static.cdninstagram.com/rsrc.php/v3/yL/r/gzOEkp8ap8N.js?_nc_x=Ij3Wp8lg5Kz',
            'lineNumber' => 228,
            'columnNumber' => 2892,
          ),
        ),
        'parentId' => 
        array (
          'id' => '51708',
          'debuggerId' => '-6751219758520329769.4700795464129842945',
        ),
      ),
    ),
  ),
  '_priority' => 'High',
  '_resourceType' => 'script',
  'cache' => 
  array (
  ),
  'pageref' => 'page_2',
  'request' => 
  array (
    'method' => 'GET',
    'url' => 'https://static.cdninstagram.com/rsrc.php/v3/yx/r/cM77mU7hqcf.js?_nc_x=Ij3Wp8lg5Kz',
    'httpVersion' => 'h3',
    'headers' => 
    array (
      0 => 
      array (
        'name' => 'sec-ch-ua',
        'value' => '"Chromium";v="124", "Google Chrome";v="124", "Not-A.Brand";v="99"',
      ),
      1 => 
      array (
        'name' => 'Referer',
        'value' => 'https://www.instagram.com/',
      ),
      2 => 
      array (
        'name' => 'Origin',
        'value' => 'https://www.instagram.com',
      ),
      3 => 
      array (
        'name' => 'sec-ch-ua-mobile',
        'value' => '?0',
      ),
      4 => 
      array (
        'name' => 'User-Agent',
        'value' => 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0.0.0 Safari/537.36',
      ),
      5 => 
      array (
        'name' => 'sec-ch-ua-platform',
        'value' => '"Windows"',
      ),
    ),
    'queryString' => 
    array (
      0 => 
      array (
        'name' => '_nc_x',
        'value' => 'Ij3Wp8lg5Kz',
      ),
    ),
    'cookies' => 
    array (
    ),
    'headersSize' => -1,
    'bodySize' => 0,
  ),
  'response' => 
  array (
    'status' => 200,
    'statusText' => '',
    'httpVersion' => 'h3',
    'headers' => 
    array (
      0 => 
      array (
        'name' => 'date',
        'value' => 'Sun, 05 May 2024 14:17:16 GMT',
      ),
      1 => 
      array (
        'name' => 'content-encoding',
        'value' => 'zstd',
      ),
      2 => 
      array (
        'name' => 'x-content-type-options',
        'value' => 'nosniff',
      ),
      3 => 
      array (
        'name' => 'content-md5',
        'value' => 'epjcQ7nVa1vjXLXkZ7+Vdg==',
      ),
      4 => 
      array (
        'name' => 'document-policy',
        'value' => 'force-load-at-top',
      ),
      5 => 
      array (
        'name' => 'cross-origin-resource-policy',
        'value' => 'cross-origin',
      ),
      6 => 
      array (
        'name' => 'alt-svc',
        'value' => 'h3=":443"; ma=86400',
      ),
      7 => 
      array (
        'name' => 'content-length',
        'value' => '568',
      ),
      8 => 
      array (
        'name' => 'reporting-endpoints',
        'value' => 'permissions_policy="https://www.facebook.com/ajax/browser_error_reports/"',
      ),
      9 => 
      array (
        'name' => 'x-fb-connection-quality',
        'value' => 'EXCELLENT; q=0.9, rtt=41, rtx=916, c=110, mss=1232, tbw=2953214, tp=2436, tpl=916, uplat=1, ullat=-1',
      ),
      10 => 
      array (
        'name' => 'x-fb-debug',
        'value' => 'JqpNw8pwBzL/1y15tx08ow+JBRmzXcfO1gXfw0MhSAtik3HKaS52toz/WERQyk8YhSG78ZF34bh3yYhPK+Iz0g==',
      ),
      11 => 
      array (
        'name' => 'last-modified',
        'value' => 'Mon, 01 Jan 2001 08:00:00 GMT',
      ),
      12 => 
      array (
        'name' => 'vary',
        'value' => 'Origin',
      ),
      13 => 
      array (
        'name' => 'report-to',
        'value' => '{"max_age":21600,"endpoints":[{"url":"https:\\/\\/www.facebook.com\\/ajax\\/browser_error_reports\\/"}],"group":"permissions_policy"}',
      ),
      14 => 
      array (
        'name' => 'content-type',
        'value' => 'application/x-javascript; charset=utf-8',
      ),
      15 => 
      array (
        'name' => 'access-control-allow-origin',
        'value' => 'https://www.instagram.com',
      ),
      16 => 
      array (
        'name' => 'cache-control',
        'value' => 'public,max-age=31536000,immutable',
      ),
      17 => 
      array (
        'name' => 'permissions-policy',
        'value' => 'accelerometer=(), attribution-reporting=(), autoplay=(), bluetooth=(), camera=(), ch-device-memory=(), ch-downlink=(), ch-dpr=(), ch-ect=(), ch-rtt=(), ch-save-data=(), ch-ua-arch=(), ch-ua-bitness=(), ch-viewport-height=(), ch-viewport-width=(), ch-width=(), clipboard-read=(), clipboard-write=(), display-capture=(), encrypted-media=(), fullscreen=(self), gamepad=(), geolocation=(), gyroscope=(), hid=(), idle-detection=(), interest-cohort=(), keyboard-map=(), local-fonts=(), magnetometer=(), microphone=(), midi=(), otp-credentials=(), payment=(), picture-in-picture=(), private-state-token-issuance=(), publickey-credentials-get=(), screen-wake-lock=(), serial=(), shared-storage=(), shared-storage-select-url=(), private-state-token-redemption=(), usb=(), window-management=(), xr-spatial-tracking=();report-to="permissions_policy"',
      ),
      18 => 
      array (
        'name' => 'timing-allow-origin',
        'value' => '*',
      ),
      19 => 
      array (
        'name' => 'priority',
        'value' => 'u=1',
      ),
      20 => 
      array (
        'name' => 'expires',
        'value' => 'Sun, 04 May 2025 00:49:08 GMT',
      ),
    ),
    'cookies' => 
    array (
    ),
    'content' => 
    array (
      'size' => 1328,
      'mimeType' => 'application/x-javascript',
      'text' => ';/*FB_PKG_DELIM*/

__d("PolarisStoriesV3HighlightsPageQuery_instagramRelayOperation",[],(function(a,b,c,d,e,f){e.exports="7869112639788839"}),null);
__d("PolarisStoriesV3HighlightsPageQuery$Parameters",["PolarisStoriesV3HighlightsPageQuery_instagramRelayOperation"],(function(a,b,c,d,e,f){"use strict";a={kind:"PreloadableConcreteRequest",params:{id:b("PolarisStoriesV3HighlightsPageQuery_instagramRelayOperation"),metadata:{is_distillery:!0},name:"PolarisStoriesV3HighlightsPageQuery",operationKind:"query",text:null}};e.exports=a}),null);
__d("PolarisStoriesV3HighlightsRoot.entrypoint",["JSResourceForInteraction","PolarisStoriesV3HighlightsPageQuery$Parameters"],(function(a,b,c,d,e,f,g){"use strict";a={getPreloadProps:function(a){var b,d=a.passthroughProps;a=a.routeProps.highlight_id;b=(b=d==null?void 0:d.initialHighlightIdParam)!=null?b:a;a=d==null?void 0:d.highlightIds;var e;a!=null?(d="Gallery",e={first:3,initial_reel_id:b,last:2,reel_ids:a}):(d="Standalone",e={first:1,initial_reel_id:b,reel_ids:[b]});return{extraProps:{highlightIds:a,initialHighlightId:b,variant:d},queries:{query:{parameters:c("PolarisStoriesV3HighlightsPageQuery$Parameters"),variables:e}}}},root:c("JSResourceForInteraction")("PolarisStoriesV3HighlightsRoot.react").__setRef("PolarisStoriesV3HighlightsRoot.entrypoint")};g["default"]=a}),98);',
    ),
    'redirectURL' => '',
    'headersSize' => -1,
    'bodySize' => 0,
    '_transferSize' => 0,
    '_error' => NULL,
  ),
  'serverIPAddress' => '157.240.227.63',
  'startedDateTime' => '2024-05-05T15:15:20.758Z',
  'time' => 66.99500000013359,
  'timings' => 
  array (
    'blocked' => 1.3470000005223555,
    'dns' => -1,
    'ssl' => -1,
    'connect' => -1,
    'send' => 0,
    'wait' => 55.129000000134866,
    'receive' => 10.518999999476364,
    '_blocked_queueing' => 1.1890000005223555,
  ),
)